...
package org.codinjutsu.tools.nosql.database.mongo.runner;
...
public class MongoRunConfigurationEditor extends SettingsEditor<MongoRunConfiguration> {
    ...
    public MongoRunConfigurationEditor(Project project) {
        ...
        DatabaseServer[] mongoServers = getAvailableServers(project);
        ...
        serverConfigurationCombobox.addItemListener(new ItemListener() {
            ...
            public void itemStateChanged(ItemEvent itemEvent) {
                ...
                for (NoSqlDatabase database: selectedServer.getDatabases()) {
    ...
    private DatabaseServer[] getAvailableServers(Project project) {
        List<DatabaseServer> mongoServers = MongoManager.getInstance(project).loadServers(NoSqlConfiguration.getInstance(project).getServerConfigurations(), false);
        List<DatabaseServer> availableServers = new LinkedList<DatabaseServer>();
        for (DatabaseServer mongoServer : mongoServers) {
            if (mongoServer.hasDatabases()) {
                availableServers.add(mongoServer);
            }
        }
        return availableServers.toArray(new DatabaseServer[availableServers.size()]);
    }
