...
public class MongoConsoleAction extends AnAction implements DumbAware {
    ...
    public void update(AnActionEvent e) {
        ...
        e.getPresentation().setVisible(configuration != null && StringUtils.isNotBlank(configuration.getShellPath(DatabaseVendor.MONGO))
                && noSqlExplorerPanel.getConfiguration() != null && noSqlExplorerPanel.getConfiguration().isSingleServer());
        e.getPresentation().setEnabled(noSqlExplorerPanel.getSelectedMongoDatabase() != null);
    }
    ...
    private void runShell(Project project) {
        MongoConsoleRunner consoleRunner = new MongoConsoleRunner(project, noSqlExplorerPanel.getConfiguration(), noSqlExplorerPanel.getSelectedMongoDatabase());
        try {
